...
struct Provider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(
            date: Date(),
            currentMl: UserDefaults(suiteName: "group.com.hanotech.justdrinkapp")?.integer(forKey: "currentMl") ?? 0,
            goalMl: UserDefaults(suiteName: "group.com.hanotech.justdrinkapp")?.integer(forKey: "goalMl") ?? 2500,
            glassSize: UserDefaults(suiteName: "group.com.hanotech.justdrinkapp")?.integer(forKey: "glassSize") ?? 250,
            glassesCount: UserDefaults(suiteName: "group.com.hanotech.justdrinkapp")?.integer(forKey: "glassesCount") ?? 0
        )
        completion(entry)
    }
...
struct AddWaterIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        _ = try? await UserDefaults(suiteName: "group.com.hanotech.justdrinkapp")?.set(amount, forKey: "lastLoggedAmount")
